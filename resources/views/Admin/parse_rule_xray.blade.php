	...
	<div class="col-sm-8">
		<div class="col-sm-12 __rule_container">
			...
			<div id="rule-form" class="form-group well" >
                <label for="">List Element <input class="form-control" type="text" id="list_element" name="list_element"></label>
...
<script>
    ...
    $(document).on('click', '#newRule , .__rule_list li span', function(event) {
        ...
        if (rule != '' && rule != null) {
            $('#rule-form input:eq(0)').val(parseRule.list[rule].list_element);
            $('#rule-form input:eq(1)').val(parseRule.list[rule].title_filter);
            $('#rule-form input:eq(2)').val(parseRule.list[rule].link_filter);
            $('#rule-form input:eq(3)').val(parseRule.list[rule].cover_filter);
            $('#rule-form input:eq(4)').val(parseRule.list[rule].cover_filter_attr);
            $('#rule-form').attr('index' , rule);
        } else {
            $('#rule-form input:eq(0)').val('');
            $('#rule-form input:eq(1)').val('');
            $('#rule-form input:eq(2)').val('');
            $('#rule-form input:eq(3)').val('');
            $('#rule-form input:eq(4)').val('src');
        }
    ...
    $(document).on('click', '#save-rule', function(event) {
        ...
        if (rule != '' && rule != null) {
            parseRule.list[rule].list_element = $('#rule-form input:eq(0)').val();
            parseRule.list[rule].title_filter = $('#rule-form input:eq(1)').val();
            parseRule.list[rule].link_filter = $('#rule-form input:eq(2)').val();
            parseRule.list[rule].cover_filter = $('#rule-form input:eq(3)').val();
            parseRule.list[rule].cover_filter_attr = $('#rule-form input:eq(4)').val();
            $(this).parent().removeAttr('index');
        } else {
            var newRule = {}
            newRule.list_element = $('#rule-form input:eq(0)').val();
            newRule.title_filter = $('#rule-form input:eq(1)').val();
            newRule.link_filter = $('#rule-form input:eq(2)').val();
            newRule.cover_filter = $('#rule-form input:eq(3)').val();
            newRule.cover_filter_attr = $('#rule-form input:eq(4)').val();
